...
export default function SegmentDetailComposeGenerator(options) {
  let { baseComposeURL, titlePrefix ='Segment', segmentform, } = options;

  return class SegmentDetailCompose extends Component {
    constructor(props) {
      super(props);
      let segmentData = {};
    ...
    editSegment(formdata) {
      ...
      request(baseComposeURL, //constants.pipelines.all.BASE_URL + constants.pipelines.segments.POST_NEW
        {
          method: 'POST',
          headers: {
        ...
        .catch(e => {
          this.props.handleErrorNotification({ message: 'Could not create a new '+titlePrefix+'. ' + e, }, e);
        });
    }
      ...
      return (
        <View style={{ flex: 1, alignSelf: 'stretch', }}>
          <ScrollView style={styles.scrollViewStandardContainer} contentContainerStyle={[ styles.scrollViewStandardContentContainer, { padding: 10, paddingBottom: 120, }]} className="segmentScrollView">
            <H2>{`New ${titlePrefix}: ${_segment.title ||''}`}</H2>
