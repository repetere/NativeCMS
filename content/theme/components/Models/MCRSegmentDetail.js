...
import { getSegmentConditionsTable, } from '../Pipelines/pipelineTableLayout';
import { getMCRSegmentRulesetRulesTable, } from './modelTableLayout';


class MCRSegmentDetail extends Component {
  constructor(props){
    super(props);
    this.state = {
    };
    ...
    return (
      <DetailView {...this.props}>
        <View style={layoutStyles.detail_widthPadding}>
          ...
          <RESPONSIVE_GRID columns={1}>
            {(this.props.detailData.ruleset && this.props.detailData.ruleset.rules && this.props.detailData.ruleset.rules.length>0)?(getMCRSegmentRulesetRulesTable.call(this, this.props.detailData.ruleset.rules||[])):(<GRID_ITEM gridItemContentStyle={{borderTopWidth:0}}><Text>No Segment Ruleset</Text></GRID_ITEM>)}
            {}
          </RESPONSIVE_GRID>
        </View>
      </DetailView>
...
export default MCRSegmentDetail;